...
let downloadPath: string = app.getPath('downloads')
const pausedTorrents = new Set<string>()
...
async function saveState() {
  ...
  try {
    ...
    console.log(`[Main] State content summary: ${JSON.stringify({ ...state, torrents: torrents.map((t:any) => ({ infoHash: t.infoHash, hasMagnet: !!t.magnetURI })) })}`)
...
async function loadState() {
  try {
    ...
    console.log(`[Main] Current client torrents count: ${torrentClient.torrents.length}`)
    ...
    for (const t of torrents) {
      ...
      const existing = torrentClient.get(t.infoHash)
      console.log(`[Main] Check ${t.infoHash}: found=${!!existing}`)
      if (existing) {
          console.log(`[Main] Existing torrent keys: ${Object.keys(existing)}`)
      }

      if (t.infoHash && !existing) {
        try {
...
async function initTorrentClient() {
  ...
  torrentClient.on('error', (err: any) => {
...
function serializeTorrent(torrent: any) {
  ...
  let torrentPath: string | null = null
...
ipcMain.handle('torrent:set-download-path', (_event, path: string) => {
  downloadPath = path
  saveState()
})
...
ipcMain.handle('torrent:get-all', () => {
  ...
  console.log(`[IPC] Sending ${torrents.length} torrents to renderer. InfoHashes: ${torrents.map((t: any) => t.infoHash).join(', ')}`)
...
ipcMain.handle('torrent:get-status', () => {
  ...
  return {
    ...
    downloading: torrents.filter((t: any) => !t.done && !pausedTorrents.has(t.infoHash)).length,
    seeding: torrents.filter((t: any) => t.done && !pausedTorrents.has(t.infoHash)).length,
    downloadSpeed: torrentClient.downloadSpeed,
    uploadSpeed: torrentClient.uploadSpeed
  }
})

ipcMain.handle('torrent:add', async (_event, magnetURI: string, downloadPath?: string) => {
  ...
  if (existing) {
    if (existing.infoHash) {
      ...
      torrentClient.remove(magnetURI, { destroyStore: false }, (err: any) => {
         if (err) console.error('[IPC] Failed to remove invalid torrent:', err)
      })
      // Continue to add new
    }
  }
...
ipcMain.handle('torrent:pause', (_event, infoHash: string) => {
...
ipcMain.handle('torrent:resume', (_event, infoHash: string) => {
  const torrent = torrentClient?.get(infoHash)
  if (torrent) {
    if (torrent.pieces) {
...
// Shared Folder Logic
async function scanAndSeed(folderPath: string) {
  try {
    ...
    for (const file of files) {
      ...
      const existing = torrentClient.torrents.some((t: any) => 
        t.files && t.files.some((f: any) => f.path === file)
      )
...
ipcMain.handle('torrent:set-share-path', async (_event, folderPath: string) => {
